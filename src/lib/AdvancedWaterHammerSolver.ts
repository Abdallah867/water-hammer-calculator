...
export class AdvancedWaterHammerSolver implements IWaterHammerSolver {
  ...
  public simulate(maxSteps: number): SimulationStep[] {
    ...
    let currentH_res = this.H_res_initial
    ...
    for (let step = 1; step <= maxSteps; step++) {
      ...
      // Stop the simulation if the reservoir is completely empty (H_res <= 0)
      if (currentH_res <= 0) {
        break
      }
      ...
      // 3. Update Reservoir Level explicitly based on flow leaving it (Node 0)
      // dH_res/dt = -Q_node0 / A_res -> H_res_new = H_res_old - (Q0_avg * dt) / A_res
      const Q_reservoir = currentNodes[0].Q
      currentH_res = currentH_res - (Q_reservoir * this.dt) / this.A_res
